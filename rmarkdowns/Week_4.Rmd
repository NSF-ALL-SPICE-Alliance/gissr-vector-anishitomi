...
```{r}
  # Note we have to rearrange the coordinates of the bounding box a little bit 
  # to give get_inat_obs() what it expects
inat_obs_df_2 <- get_inat_obs(taxon_id = 122835, #taxon id from URL
                            place_id = 1856, #place id from URL
                            quality = "research", #specifies research grade only
                            geo = TRUE, #Specifies that we want geocoordinates
                            maxresults = 100) #Limits results

save(inat_obs_df_2, file = "oahu_taro_inat.Rdata")
```

```{r}
inat_obs_sf_2 <-  inat_obs_df_2 %>% 
  select(longitude, latitude, datetime, common_name, scientific_name, image_url, user_login) %>% 
  st_as_sf(coords=c("longitude", "latitude"), crs=4326)
...
Scatter plot of data points of Taro on Oahu
```{r}
ggplot() +
  geom_point(data = inat_obs_df_2, aes(x = longitude, y = latitude, color = common_name, text = common_name)) 
```
...
Retreaving data on the different parts of Oahu
```{r}
...
p2 <- ggplot() +
  ...
  geom_point(data = inat_obs_df_2, aes(x = longitude, y = latitude, color = common_name, text = common_name)) + # here is our iNaturalist data
  ...
  ggtitle("Taro on Oahu", subtitle = "Based on iNaturalist Data as of October 2024") +
  theme_bw() +
  labs(
    color="Common Name", #this changes the title of our legend
    x = "Longitude",
...
```{r}
inat_obs_popup_sf_2 <- inat_obs_sf_2 %>% 
  mutate(popup_html = paste0("<p><b>", common_name, "</b><br/>",
                             "<i>", scientific_name, "</i></p>",
                             "<p>Observed: ", datetime, "<br/>",
                             "User: ", user_login, "</p>",
...
```{r}
htmltools::p("iNaturalist Observations of Taros on the Island of Oahu",
             htmltools::br(),
             inat_obs_popup_sf$datetime %>% 
               as.Date() %>% 
               range(na.rm = TRUE) %>% 
...
```{r}
leaflet(inat_obs_sf_2) %>% 
  setView(lng = -157.8, lat = 21.33, zoom = 12)%>%
  addTiles() %>% 
  addCircleMarkers(data = inat_obs_popup_sf_2,
                   popup = ~popup_html, 
                   radius = 5)
```
