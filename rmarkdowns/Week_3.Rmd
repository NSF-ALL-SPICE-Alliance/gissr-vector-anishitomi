...
Runnning the same code above, but for my variable I created
```{r}
boundary_box %>% 
  opq()
```
...
Runnning the same code as above, but for my new boundary box. Except I am using a different key and value
```{r}
...
ggplot() +
  ...
  coord_sf(xlim = c(-156.69726, -157.64863), ylim = c(20.57443, 21.03156), expand = FALSE)
```

Performing Challenge 3
```{r}
ggplot() +
  geom_sf(data = prisons$osm_polygons) +
...
ggplot() +
  ...
  geom_sf(data = small_streets$osm_lines, inherit.aes = FALSE, color = "#a6a6a6", size = .2, alpha = .8) +
  geom_sf(data = coastline$osm_lines, inherit.aes = FALSE, color = "black", size = .8, alpha = .5) +
  geom_sf(data = prisons$osm_polygons, inherit.aes = FALSE, colour = "red", fill = "red", alpha = .5, size = 1) +
  coord_sf(xlim = c(-158.28058, -157.64863), ylim = c(21.25482, 21.71201), expand = TRUE) +
  geom_sf_text(data = prisons$osm_polygons, aes(label = name), size = 2, hjust = 0) +
  theme_bw() +
  labs(
    title = "Prisons on Oahu",
    x = "Longitude",
    y = "Latitude"
  )
```
...
Performing Challenge 6
```{r}
leaflet() %>%
  addTiles() %>%
  addPolygons(
    data = prisons$osm_polygons,
    label = prisons$osm_polygons$name
  ) 
```
...
Since I am looking at the island of Oahu and the fact that there is a major overcrowding problem within the prison system, what if I were to Identify the optimal locations for new prison facility based on population density, land availability, and proximity to essential services (e.g., courts, hospitals).  Or the other idea I had was to assess the accessibility and efficiency of public transportation systems in Oahu. 
